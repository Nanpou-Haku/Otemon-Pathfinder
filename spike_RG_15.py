...
def UpdateCamera(stream, kernel):
    ...
    blind = cv2.rectangle(hsvline, (0, 0), (320, 120), (100, 100, 100), -1)
...
def distanceTraceMove(motor, nowPosition, goPosition):
    if(nowPosition != goPosition):
        motor.run_to_position(goPosition)
        nowPosition = goPosition
...
def distanceMove(steering, nowPosition, left):
    ...
    global pole, counter, blueFlag, poleFlag, first, waitPls
    ...
    if first is True:
        if cm_front < 40:
            ...
            if blueFlag is False and waitPls is True:
                cm_side = read_distance_side()
                time.sleep(0.05)
                print("------")
                print("cm_side =", cm_side)
                print("tyuppatyappusu")
    
                if cm_side <= 30:
                    nowPosition = distanceTraceMove(steering, nowPosition, -30)
        
                elif 30 < cm_side < 40:
                    nowPosition = distanceTraceMove(steering, nowPosition, 0)
...
def back(nowPosition, color):
    global total, clock, counter, surf, blueFlag, poleFlag, waitPls
    ...
    while True:
        ...
        if total == 0:
            ...
            if red.area < green.area:
                color = "green"
                print("------")
                print("obstacle = GREEN")
                print("G area = ", green.area)
                print("R area = ", red.area)
